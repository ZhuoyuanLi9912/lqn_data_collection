    ...
    % first layer
    P1 = Processor(model, 'Layer1', 3, SchedStrategy.PS);
    T1 = Task(model, 'Client1', 6, SchedStrategy.REF).on(P1);
    E1 = Entry(model, 'Browse1').on(T1);
    T2 = Task(model, 'Client2', 6, SchedStrategy.REF).on(P1);
    E2 = Entry(model, 'Browse2').on(T2);
    T3 = Task(model, 'Client3', 6, SchedStrategy.REF).on(P1);
    E3 = Entry(model, 'Browse3').on(T3);
        
        
    %second layer
    P2 = Processor(model, 'Layer2', 2, SchedStrategy.PS);
    T4 = Task(model, 'Router', 5, SchedStrategy.FCFS).on(P2);
    ...
    T6 = Task(model, 'Cart', 3, SchedStrategy.FCFS).on(P4);
    ...
    T8 = Task(model, 'CartDB', 2, SchedStrategy.FCFS).on(P5);
    E13 = Entry(model, 'queryCart').on(T8);
    T9 = Task(model, 'CatDB', 1, SchedStrategy.FCFS).on(P5);
    ...
    A5 = Activity(model, 'A5', Exp.fitMean(0.3)).on(T4).synchCall(E5,0.5).repliesTo(E4);
    A6 = Activity(model, 'A6', Exp.fitMean(0.3)).on(T4).synchCall(E6,0.5).repliesTo(E4);
    A7 = Activity(model, 'A7', Exp.fitMean(0.3)).on(T4).synchCall(E7,0.5).repliesTo(E4);
    T4.addPrecedence(ActivityPrecedence.OrFork(A4,{A5,A6,A7},[0.4,0.3,0.3]));

    A8 = Activity(model, 'A8', Exp.fitMean(2.1)).on(T5).boundTo(E5).repliesTo(E5);
    
    A9 = Activity(model, 'A9', Exp.fitMean(3.1)).on(T5).boundTo(E6);
    A10 = Activity(model, 'A10', Exp.fitMean(0.3)).on(T5).synchCall(E8,3).repliesTo(E6);
    A11 = Activity(model, 'A11', Exp.fitMean(0.3)).on(T5).synchCall(E9,3).repliesTo(E6);
    A12 = Activity(model, 'A12', Exp.fitMean(0.3)).on(T5).synchCall(E10,3).repliesTo(E6);
    T5.addPrecedence(ActivityPrecedence.OrFork(A9,{A10,A11,A12},[0.3,0.3,0.4]));

    A13 = Activity(model, 'A13', Exp.fitMean(3.7)).on(T5).boundTo(E7);
    A14 = Activity(model, 'A14', Exp.fitMean(0.3)).on(T5).synchCall(E11,0.3).repliesTo(E7);
    A15 = Activity(model, 'A15', Exp.fitMean(0.3)).on(T5).synchCall(E12,0.3).repliesTo(E7);
    T5.addPrecedence(ActivityPrecedence.OrFork(A13,{A14,A15},[0.5,0.5]));

    A16 = Activity(model, 'A16', Exp.fitMean(4.4)).on(T6).boundTo(E8).synchCall(E13,1).repliesTo(E8);
    A17 = Activity(model, 'A17', Exp.fitMean(3.6)).on(T6).boundTo(E9).synchCall(E13,1).repliesTo(E9);
