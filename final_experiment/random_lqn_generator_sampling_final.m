function LQN = random_lqn_generator_sampling_final(sobolCSV, lookupCSV, outputMATname, logfileName)
    basePath = "C:\GLQN\matlab\data\final_experiment";
...
function results = parse_csv_file(sobolCSV, lookupCSV)
    % This will spin up Python in-process and load the module
    np = py.importlib.import_module("numpy");

    % Replace with the full path if needed
    pyArr = np.load(sobolCSV);

    % Now convert to a native MATLAB matrix of doubles
    sobol = double(pyArr);
    ...
    for i = 1:size(sobol, 1)
        ...
        service_times_mean = row(59:178);
        service_times_scv = row(179:298);
        call_times = row(299:388);
        ...
        try
            ...
            for i = 1:result.processor_count-1
                ...
                while sum(result.calls_per_entry(current_layer_entries)) < length(next_layer_entries)
                    ...
                    if result.calls_per_entry(entry)>4
            ...
            LQN(successful_count).entry_queue_lengths                 = mean_queue_lengths;
            LQN(successful_count).entry_response_times                = mean_response_times;
            LQN(successful_count).entry_throughputs                   = mean_throughputs;
